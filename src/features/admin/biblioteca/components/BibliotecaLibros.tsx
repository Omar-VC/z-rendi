...
export default function BibliotecaLibros({ preparadorId }: Props) {
  const { libros, loading, agregarLibro, recargar } =
    ...
    <div className="space-y-4">
      ...
      {!loading && libros.length === 0 && (
        <div className="border rounded p-6 text-center">
          <p className="text-gray-500">Todavía no hay libros creados.</p>
      ...
      {!loading && libros.length > 0 && (
        <div className="grid gap-4">
          {libros.map((libro) => (
            <LibroCard key={libro.id} libro={libro} onActualizado={recargar} />
          ))}
        </div>
      )}
