...
const SesionesCliente = ({ clienteId }: { clienteId: string }) => {
  ...
  const sesionesPendientes = sesiones.filter((s) => s.estado !== "completada");
  ...
  return (
    <div className="space-y-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <div className="card w-full max-w-md">
            <h3 className="font-semibold mb-4 text-white">Nueva sesión</h3>
            <input
              type="date"
            ...
            <textarea
              ...
              onChange={(e) =>
                setFormData({ ...formData, bloques: e.target.value })
              }
              className="input mb-3"
            />
      ...
      {/* Lista de pendientes */}
      <div>
        <h3 className="font-bold text-white mb-2">Sesiones pendientes</h3>
        <SesionList
      ...
      {/* Detalle de sesión (pendiente o completada) */}
      <SesionDetail
