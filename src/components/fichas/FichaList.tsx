    ...
    <>
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {fichas.map((ficha) => {
          ...
          return (
            ...
            >
              <button className="w-full text-left text-white" type="button" onClick={() => handleSelect(ficha)}>
                <p className="font-semibold truncate">{ficha.nombre} {ficha.apellido}</p>
                <p className="text-sm text-gray-300">{ficha.puesto ?? "-"}</p>
                <p className="text-sm text-gray-400">{ficha.edad} años · {ficha.peso} kg · {ficha.altura} cm</p>
              </button>
            </li>
      ...
      {isModalOpen && selectedFicha && (
        <Modal onClose={() => setIsModalOpen(false)}>
          ...
          <p><strong>Puesto:</strong> {selectedFicha.puesto ?? "-"}</p>
          ...
          <button onClick={() => setIsModalOpen(false)} className="mt-4 bg-gray-500 text-white px-3 py-1 rounded">
            Cerrar
          </button>
        </Modal>
      )}
